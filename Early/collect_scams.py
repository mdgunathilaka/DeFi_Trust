...
INFURA_URL = 'https://mainnet.infura.io/v3/45fd456a1bc349108a08f6c40a23b78c'
...
def get_logs(contract, myevent, hash_create, from_block, block_range=1000):
    #print("get_logs")
    """
...
df = pd.read_csv('labeled_list.csv')

scam_df = df.loc[df['label']==0]
#print(scam_df.head())
...
w = 0
for i in scam_df['token_address']:
    print(w, i)
    len_t = get_transfers(str(i), './scam_event_series', 1)
    count_df = pd.DataFrame(list(zip(token_names, counts)),columns=['token_address', 'event_count'])
    w+=1

# saving the dataframe
count_df.to_csv('count_events.csv')
